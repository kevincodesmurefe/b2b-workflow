...
interface UpdatePasswordBody { password: string; }
...
export const updateUserPasswordController = async (req: Request<Id, {}, UpdatePasswordBody>, res: Response, next: NextFunction): Promise<void> => {
    const tenantId = req.user!.tenantId;
    try {
        const user = await userService.updateUserPassword(Number(req.params.id), tenantId, req.body.password);
        res.status(200).json(user);
    } catch (error) {
        next(error);
    }
