...
export const generateAccessToken = (user: Users, sessionId: number, tokenVersion: number): string => {
    return jwt.sign({sessionId: sessionId, tokenVersion: tokenVersion, userId: user.id, role: user.role, tenantId: user.tenantId, jti: crypto.randomUUID()}, config.secrets.jwtSecret, { expiresIn: "15m" });
}
